...
;;     (defvar helm-type-buffer-actions-saved helm-type-buffer-actions)
...
;;     (defun helm-switch-to-buffers-in-persp (buffer-or-name)
;;       (let* ((mkds (helm-marked-candidates))
;;              (buf (get-buffer (or (car mkds) buffer-or-name)))
;;              (source (helm-get-current-source))
;;              (htc (helm-this-command)))
;;         (if (and (or (eq htc 'helm-persp-mini)
;;                      (eq htc 'helm-mini))
;;                  (buffer-live-p buf))
;;             (let ((persp (cdr (assoc 'persp source))))
;;               (if persp
;;                   (progn
;;                     (persp/ui/frame-switch (persp/ll/macro/persp-name persp))
...
;;     (defvar helm-persp-buffer-actions
;;       (helm-make-actions
;;        "Switch to buffer in perspecive" 'helm-switch-to-buffers-in-persp))

;;     (defvar helm-persp-buffer-actions-combined
;;       (append helm-persp-buffer-actions helm-type-buffer-actions))

;;     (defclass helm-persp-free-buffers-source (helm-source-buffers)
;;       ((buffer-list
;;         :initarg :buffer-list
;;         :initform #'(lambda () (mapcar #'buffer-name (persp-buffer-list-restricted nil 3 nil t)))
;;         :custom function
;;         :documentation
;;         "  A function with no arguments to create buffer list.")))
...
;;     (defmethod helm--setup-source :after ((source helm-persp-buffers-source))
;;       (set-slot-value source 'action helm-persp-buffer-actions-combined))
...
;;     (add-hook 'persp/custom/persp/created-functions
;;               #'(lambda (p ph)
;;                   (when (and (eq ph persp/var/*persp-hash*) p)
;;                     (let* ((pn (persp/ll/macro/persp-name p))
;;                            (source-name (persp-helm--soure-name-from-persp-name pn)))
;;                       (eval
;;                        `(defvar ,source-name nil))
;;                       (set source-name (helm-make-source (concat pn " buffers")
...
;;     (add-hook 'persp/custom/persp/before-kill-functions
;;               #'(lambda (p)
;;                   (when p
;;                     (let* ((pn (persp/ll/macro/persp-name p))
;;                            (source-name (persp-helm--soure-name-from-persp-name pn)))
...
;;             (let ((source-name (persp-helm--soure-name-from-persp-name (persp/ll/macro/persp-name persp))))
;;               (persp-helm--liftup-source source-name))))
;;         (or
;;          (helm :sources helm-mini-default-sources
;;                :ff-transformer-show-only-basename nil
