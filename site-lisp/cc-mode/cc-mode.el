...
(defun c-before-change (beg end)
  ...
  (unless (c-called-from-text-property-change-p)
	...
	(unwind-protect
		...
		;; makes a type a type?  E.g. by inserting stuff after "foo"
		;; in "foo bar;", or before "foo" in "typedef foo *bar;"?
		;;
		...
		(let (lim lim-2
		  ...
		  (when (>= end1 beg) ; Don't hassle about changes entirely in
			...
			;; Get the identifier, if any, that the property is
			;; on.
  ...
  (unless (c-called-from-text-property-change-p)
	...
	(save-match-data	  ; c-recognize-<>-arglists changes match-data
	  ...
	  (unwind-protect
		...
		;; c-</>-as-paren-syntax and c-cpp-delimiter into hard
		;; syntax-table properties.  Remove these when it happens.
